...
const defaultProfile = { name: 'PGS.TS. Huỳnh Công Pháp', subtitle: 'Giảng viên đại học • Toán, Hóa', bio: 'PGS.TS Huỳnh Công Pháp có hơn 15 năm giảng dạy...', education: 'Tiến sĩ Toán - Đại học X', experience: '15 năm giảng dạy', languages: 'Tiếng Việt', formats: 'Tại nhà / Online', price: '200k / buổi', subjects: [{ subject: 'Toán', grade: 'Lớp 10-12' }] };
...
function renderProfile(p) {
    ...
    p.subjects.forEach((s, idx) => {
        ...
        row.innerHTML = `<span class="sub">${
            escapeHtml(s.subject)
        }
        </span> <span class="grade">${
            escapeHtml(s.grade)
        }
        </span> <button data-i="${idx}" class="btn-small remove">Xoá</button>`; 
...
let profile = loadProfile(); renderProfile(profile);
...
if (!isTutor) {
    ...
    if (main) {
        main.innerHTML = `<div class="private-note"><h3>Trang này chỉ dành cho gia sư</h3><p>Bạn cần đăng nhập với tài khoản gia sư để xem và chỉnh sửa hồ sơ cá nhân.</p><div class="private-actions"><button id="mockLogin" class="btn-primary">Mô phỏng đăng nhập gia sư</button><a href="find_tutor.html" class="btn-outline">Quay lại trang tìm gia sư</a></div></div>`;
...
let editing = false;
function enterEdit() {
    editing = true; editToggle.textContent = 'Đang chỉnh sửa...'; 
...
function exitEdit(cancel = false) {
    ...
    fields.forEach(id => { 
        ...
        el.id = id; el.textContent = profile[id]; 
        if (id === 'price') 
            el.className = 'price'; 
        input.parentNode.replaceChild(el, input); 
    });
    renderProfile(profile); 
    editing = false; editToggle.textContent = 'Chỉnh sửa hồ sơ'; 
    editToggle.disabled = false; saveBtn.style.display = 'none'; 
...
if (addBtn) 
    addBtn.addEventListener('click', () => { 
        ...
        profile.subjects.push({ 
            subject: subj, grade: grade || 'Tất cả' 
        }); 
        saveProfile(profile); 
        renderProfile(profile); 
        newSubject.value = ''; 
